...
# Use the full-body Xis image supplied by the user, preserving the whole portrait.
replace_once(
    'AppV020.js',
...
replace_once('AppV020.js','<XisOfficial height={118}/>','<XisOfficial height={150}/>','larger Xis in voice stage')
...
replace_once('XisEngine.js',"import * as SecureStore from 'expo-secure-store';","import * as SecureStore from 'expo-secure-store';\nimport candidates from './tse_candidates_2026.json';",'candidate dataset import')
...
replace_once('XisEngine.js',"if(/RAIO|CANDIDAT|QUEM E|QUEM É|PROCUR|BUSC/.test(q)){","if(/RAIO|PROCUR|BUSC|PESQUIS|ABRIR|ABRA/.test(q)){",'only explicit navigation should open Raio-X')
replace_once('XisEngine.js',"if(!q)return {type:'answer',text:'Escreva sua dúvida e eu tento resolver sem usar IA.'};\n  if(/COMPAR/.test(q))","if(!q)return {type:'answer',text:'Escreva sua dúvida e eu tento resolver sem usar IA.'};\n  const listAnswer=candidateListAnswer(raw);if(listAnswer)return listAnswer;\n  if(/COMPAR/.test(q))",'candidate list answer priority')
replace_once('XisEngine.js',"if(/BUSCA POR CARGO|LISTAR|ESCOLHER CARGO|PESQUISA POR CARGO/.test(q))","if(/BUSCA POR CARGO|ESCOLHER CARGO|PESQUISA POR CARGO/.test(q))",'do not navigate just because user said listar')
...
if not asset.exists() or asset.stat().st_size<1000: raise SystemExit('Full-body Xis v0.3.27 asset missing')
print('RAIO-X v0.3.27: voice answer stays in chat + full-body Xis + larger talk button')
