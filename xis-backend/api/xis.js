...
// Warm-instance protection. The app also enforces the same limits per Firebase UID.
// This server guard prevents accidental bursts even before a persistent store is added.
...
function nowDay(t=Date.now()){
  // Limits follow Brazil time sufficiently for this app; hour guard is rolling.
...
function compactContext(input){
  ...
  return {
    screen:clean(c.screen,40),
    dataSource:'TSE 2026',
    candidate:cand?{
      id:clean(cand.id,40),name:clean(cand.name,100),civilName:clean(cand.civilName,120),number:clean(cand.number,10),
      office:clean(cand.office,60),uf:clean(cand.uf,2),party:clean(cand.party,30),partyName:clean(cand.partyName,100),
      status:clean(cand.status,160),assetTotal:Number.isFinite(Number(cand.assetTotal))?Number(cand.assetTotal):null,
      assetCount:Number.isFinite(Number(cand.assetCount))?Number(cand.assetCount):null,
      occupation:clean(cand.occupation,100),education:clean(cand.education,100),birthDate:clean(cand.birthDate,20),
      federation:clean(cand.federation,120),coalition:clean(cand.coalition,180),reelection:clean(cand.reelection,30)
    }:null
  };
}
async function verifyFirebase(idToken){
  ...
  if(!r.ok||!Array.isArray(d.users)||!d.users[0]?.localId) throw new Error('AUTH_INVALID');
  return {uid:String(d.users[0].localId),email:String(d.users[0].email||'')};
}
...
function rollback(uid){const e=rate.get(uid)||[];if(e.length){e.pop();rate.set(uid,e)}}
...
export default async function handler(req,res){
  ...
  try{
    ...
    const d=await r.json().catch(()=>({}));
    if(!r.ok){rollback(user.uid);return json(res,502,{ok:false,error:'OPENAI_ERROR',detail:clean(d?.error?.message||r.status,300)})}
    const answer=extractText(d);
    if(!answer){rollback(user.uid);return json(res,502,{ok:false,error:'EMPTY_MODEL_RESPONSE'})}
    return json(res,200,{ok:true,answer:answer.slice(0,1800),model:MODEL,remaining:slot.remaining,usage:d.usage||null});
  }catch(e){rollback(user.uid);return json(res,502,{ok:false,error:'OPENAI_UNAVAILABLE',detail:clean(e?.message||e,240)})}
}
