...
export default async function handler(req,res){
  ...
  try{
    const polls=await searchLatest(office,uf);
    if(polls.length)return send(res,200,{ok:true,fresh:true,office,uf,fetchedAt,polls});
    const fallback=office==='PRESIDENTE'?PRESIDENT_FALLBACK.map(p=>({...p,office:'PRESIDENTE',uf:'BR'})):[];
    return send(res,200,{ok:true,fresh:false,office,uf,fetchedAt,polls:fallback,warning:'Nenhuma pesquisa nova confiavel encontrada; exibindo ultima carga valida.'});
  }catch(e){
    const fallback=office==='PRESIDENTE'?PRESIDENT_FALLBACK.map(p=>({...p,office:'PRESIDENTE',uf:'BR'})):[];
    return send(res,200,{ok:true,fresh:false,office,uf,fetchedAt,polls:fallback,warning:'Atualizacao online indisponivel; exibindo ultima carga valida.',detail:clean(e?.message||e,160)});
  }
}
